#!/usr/bin/env bash
# =============================================================================
# build_intel.sh — 构建 rawengine macOS Intel (x86_64) 并收集产物
#
# 用法:
#   bash build_intel.sh [--jobs N] [--skip-build] [--clean] [--with-system-libraw]
#
# 前提:
#   需要 Intel (x86_64) 版本的 Homebrew，通常安装在 /usr/local
#   在 Apple Silicon Mac 上需要先安装 Intel Homebrew:
#     arch -x86_64 /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"
#   再安装依赖:
#     arch -x86_64 /usr/local/bin/brew install cmake ninja gtk+3 gtkmm3 exiv2 little-cms2 fftw lensfun librsvg cairomm libsigc++ glibmm libiptcdata gtk-mac-integration jpeg-xl
#
# 产物目录结构:
#   installer/output/
#     include/          — C API 头文件（arm/intel 共享）
#     dylib/intel/      — rawengine.dylib (x86_64) + 运行时依赖 dylib
#     dsym/intel/       — rawengine.dylib.dSYM 调试符号包
...
if [[ ! -x "$CMAKE_BIN" ]]; then
    ...
    echo ""
    echo "  Intel (x86_64) Homebrew is required. Install it with:"
    ...
    echo "    arch -x86_64 /usr/local/bin/brew install cmake ninja gtk+3 gtkmm3 exiv2 little-cms2 fftw lensfun librsvg cairomm libsigc++ glibmm libiptcdata gtk-mac-integration jpeg-xl"
    exit 1
fi
if [[ ! -x "$HOMEBREW_PREFIX/bin/ninja" ]]; then
    echo "[ERROR] ninja not found at $HOMEBREW_PREFIX/bin/ninja"
    echo "        Install: arch -x86_64 /usr/local/bin/brew install ninja"
    exit 1
fi
...
_LIBFFI_FIX_DIR="/tmp/rt_pkgconfig_fix_intel"
...
if [[ ${#MISSING_DEPS[@]} -gt 0 ]]; then
    echo "[ERROR] Missing pkg-config packages (x86_64): ${MISSING_DEPS[*]}"
    echo ""
    echo "  Install all required dependencies via Intel Homebrew:"
    echo "    arch -x86_64 /usr/local/bin/brew install gtk+3 gtkmm3 exiv2 little-cms2 fftw lensfun librsvg"
    echo "    arch -x86_64 /usr/local/bin/brew install cairomm libsigc++ glibmm libiptcdata gtk-mac-integration jpeg-xl"
    if [[ $WITH_SYSTEM_LIBRAW == ON ]]; then
        echo "    arch -x86_64 /usr/local/bin/brew install libraw"
    fi
    exit 1
fi

echo "============================================================"
echo " rawengine macOS Intel (x86_64) build & collect"
...
DYLIB_SEEN="$SCRIPT_DIR/.intel_dylibs_seen_$$"
: > "$DYLIB_SEEN"
...
echo "  Collected ${DYLIB_COUNT} dylib(s) in dylib/intel/"
...
if command -v dsymutil &>/dev/null; then
    ...
    echo "  rawengine.dylib → dsym/intel/rawengine.dylib.dSYM"
    ...
    if [[ -f "$DYLIB_DIR/rawengine-cli" ]]; then
        ...
        echo "  rawengine-cli   → dsym/intel/rawengine-cli.dSYM"
    fi
else
    echo "  [warn] dsymutil not found (install Xcode Command Line Tools)"
fi
...
echo ""
echo "============================================================"
echo " Output collected successfully (x86_64):"
echo "  include/         : $(find "$OUTPUT_DIR/include" -name "*.h" | wc -l | tr -d ' ') header(s)"
echo "  dylib/intel/     : ${DYLIB_COUNT} dylib(s)"
echo "  dsym/intel/      : ${DSYM_COUNT} dSYM bundle(s)"
